...
- Durable ExecPlans live under [`docs/plans/`](docs/plans/README.md)
  (`active/`, `completed/`, `deferred/`, `superseded/`).
  Authoring rules: [`docs/plans/PLANS.md`](docs/plans/PLANS.md).
- Session checklist: [`docs/process/agent-session.md`](docs/process/agent-session.md).
  Conventions: [`docs/process/todo-conventions.md`](docs/process/todo-conventions.md).
- User-facing changes → [`CHANGELOG.md`](CHANGELOG.md); harness/CI/refactors/process →
  [`MAINTENANCE.md`](MAINTENANCE.md). Semver notes: [`docs/process/semver.md`](docs/process/semver.md).
- Substantive plans must be in the repo (not only gitignored `tmp/`). On ship/defer/supersede:
  remove the TODO item, archive the plan, and log on the **same PR**
  (`python3 scripts/archive_plan.py`).
- When writing complex features or significant refactors, use an ExecPlan
  (as described in `docs/plans/PLANS.md`) from design to implementation.
- Local hooks: `git config core.hooksPath .githooks` (includes `pre-push`,
  `pre-commit` shim, and soft `commit-msg` hint for active plans).
...
- This repository is `kgforais1/pr-agent-kgforais1`, a standalone repo detached from `The-PR-Agent/pr-agent`.
- **Never create a pull request with a base outside `kgforais1/pr-agent-kgforais1`.** Never pass `--repo`/`-R` pointing at `The-PR-Agent/*`, and never create, edit, or comment on upstream issues/PRs. The sole sanctioned upstream API access is the read-only search performed by the `upstream-pr-leak-check` workflow.
...
- Never push directly to `main`: the repo ruleset requires pull requests, and `.githooks/pre-push` blocks direct pushes locally. The sole exception is the `publish.yml` release workflow's automated version-bump commit (server-side, `github-actions[bot]`); its documented manual fallback must route through the no-bypass approval rule above. Enable the hooks in every fresh clone with `git config core.hooksPath .githooks`; work on a feature branch and open a PR.
