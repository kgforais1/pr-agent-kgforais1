...
- Never push directly to `main`: the repo ruleset requires pull requests, and `.githooks/pre-push` blocks direct pushes locally. Enable the hooks in every fresh clone with `git config core.hooksPath .githooks`; work on a feature branch and open a PR.
